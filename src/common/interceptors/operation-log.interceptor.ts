...
const TARGET_LABELS: [string, string][] = [
  // 完整 URL 前缀
  ['POST /api/menu-roles/reset', 'POST /菜单权限/重置'],
  ['PUT /api/menu-roles', 'PUT /菜单权限'],
  ['POST /api/menu-roles', 'POST /菜单权限'],
  ['DELETE /api/menu-roles', 'DELETE /菜单权限'],
  ['PUT /api/configs', 'PUT /系统配置'],
  ['POST /api/configs', 'POST /系统配置'],
  ['PUT /api/admins', 'PUT /管理员'],
  ['POST /api/admins', 'POST /管理员'],
  ['PUT /api/admins/', 'PUT /管理员/'],
  ['DELETE /api/admins/', 'DELETE /管理员/'],
  ['PUT /api/dispatch/config', 'PUT /派单/配置'],
  ['POST /api/dispatch/priorities/batch', 'POST /派单/批量设置优先级'],
  ['PUT /api/dispatch/priorities/', 'PUT /派单/优先级/'],
  ['POST /api/notifications/read', 'POST /通知/标记已读'],
  ['PUT /api/notifications/', 'PUT /通知/'],
  ['PUT /api/notifications/', 'PUT /通知/'],
  ['PUT /api/faq/', 'PUT /问答/'],
  ['POST /api/faq/category', 'POST /问答/分类'],
  ['PUT /api/faq/category/', 'PUT /问答/分类/'],
  ['DELETE /api/faq/category/', 'DELETE /问答/分类/'],
  ['PUT /api/faqs/', 'PUT /问答/'],
  ['DELETE /api/faqs/', 'DELETE /问答/'],
  ['PUT /api/reviews/', 'PUT /评价/'],
  ['DELETE /api/reviews/', 'DELETE /评价/'],
  ['POST /api/reviews/', 'POST /评价/'],
  ['PUT /api/questions/', 'PUT /问答咨询/'],
  ['DELETE /api/questions/', 'DELETE /问答咨询/'],
  ['POST /api/questions/', 'POST /问答咨询/'],
  ['POST /api/questions//replies', 'POST /问答咨询/回复'],
  ['PUT /api/questions//replies/', 'PUT /问答咨询/回复/'],
  ['POST /api/content/banners', 'POST /内容/横幅'],
  ['PUT /api/content/banners/', 'PUT /内容/横幅/'],
  ['DELETE /api/content/banners/', 'DELETE /内容/横幅/'],
  ['POST /api/content/announcements', 'POST /内容/公告'],
  ['PUT /api/content/announcements/', 'PUT /内容/公告/'],
  ['DELETE /api/content/announcements/', 'DELETE /内容/公告/'],
  ['POST /api/content/intros', 'POST /内容/介绍'],
  ['PUT /api/content/intros/', 'PUT /内容/介绍/'],
  ['DELETE /api/content/intros/', 'DELETE /内容/介绍/'],
  ['PUT /api/deliveries/', 'PUT /配送/'],
  ['PUT /api/users/', 'PUT /用户/'],
  ['PUT /api/users//addresses/', 'PUT /用户//地址/'],
  ['POST /api/users//addresses', 'POST /用户//地址'],
  ['DELETE /api/users//addresses/', 'DELETE /用户//地址/'],
  ['PUT /api/outlets/', 'PUT /网点/'],
  ['POST /api/outlets/', 'POST /网点/'],
  ['PUT /api/outlets//reset-password', 'PUT /网点//重置密码'],
  ['PUT /api/outlets//business-types', 'PUT /网点//业务类型'],
  ['PUT /api/outlets//subscribe-toggle', 'PUT /网点//订阅开关'],
  ['PUT /api/outlets//bind-openid', 'PUT /网点//绑定OpenID'],
  ['PUT /api/outlets//me/password', 'PUT /网点//密码'],
  ['PUT /api/orders//assign', 'PUT /订单//分配'],
  ['PUT /api/orders//accept', 'PUT /订单//接单'],
  ['PUT /api/orders//deliver', 'PUT /订单//发货'],
  ['PUT /api/orders//sign', 'PUT /订单//签收'],
  ['PUT /api/orders//pay', 'PUT /订单//支付'],
  ['POST /api/orders//dev-paid', 'POST /订单//模拟支付'],
  ['PUT /api/orders//cancel', 'PUT /订单//取消'],
  ['PUT /api/orders//refund', 'PUT /订单//退款'],
  ['POST /api/orders//refund-request', 'POST /订单//退款申请'],
  // 通用路径片段
  ['/api/menu-roles', '/菜单权限'],
  ['/api/configs', '/系统配置'],
  ['/api/admins', '/管理员'],
  ['/api/dispatch/config', '/派单/配置'],
  ['/api/dispatch/priorities/', '/派单/优先级/'],
